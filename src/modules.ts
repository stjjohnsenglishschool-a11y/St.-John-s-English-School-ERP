...
export const ALL_SUBMENU_MODULES = [
  ...
  { key: 'fees_structure', label: 'Fees Structure', group: 'Master Setup' },
  { key: 'income_head_master', label: 'Income Heads / Types', group: 'Master Setup' },
...
export const modules: Record<string, Module> = {
  ...
  'fees_structure': {
    title: 'fees_structure',
    group: 'Master Setup',
    table: 'fees_structure',
    primaryKey: 'fee_struct_id',
    description: 'Class-wise fee structure, monthly tuition amounts, billing frequency and due dates',
    fields: [
      f('class_name', 'Class name', 'select', true, [
        'PG',
        'NURSERY',
      ...
      f('fee_type', 'Fee type', 'select', true, [
        'Monthly Tuition Fee',
        'Admission Fee',
        'Annual / Session Fee',
        'Examination Fee',
      ...
      f('amount', 'Fee amount (₹)', 'number', true),
      f('frequency', 'Billing frequency', 'select', false, ['Monthly', 'Quarterly', 'Half-Yearly', 'Annually', 'One-Time']),
      f('due_day', 'Due day of month', 'number', false),
      f('academic_year', 'Academic year', 'select', false, ACADEMIC_YEAR_OPTIONS),
      f('remarks', 'Description / Remarks', 'textarea'),
    ],
    columns: ['class_name', 'fee_type', 'amount', 'frequency', 'due_day', 'academic_year'],
  },
  'income_head_master': {
    title: 'income_head_master',
    group: 'Master Setup',
    table: 'income_head_master',
    primaryKey: 'head_id',
    description: 'Income heads, categories and fee/non-fee revenue item types',
    fields: [
      f('head_category', 'Income Group / Category', 'select', true, [
        'Fee Income',
      ...
      f('head_name', 'Income Head / Type Name', 'text', true),
      f('head_code', 'Head Code', 'text'),
      f('default_amount', 'Default Rate / Fee (₹)', 'number'),
      f('frequency', 'Frequency', 'select', false, ['As Needed', 'Monthly', 'Quarterly', 'Annual', 'One-time']),
      f('description', 'Description', 'textarea'),
      f('is_active', 'Active Status', 'boolean'),
    ],
    columns: ['head_category', 'head_name', 'head_code', 'default_amount', 'frequency', 'is_active'],
  },
  'vendor_master': {
    title: 'vendor_master',
    group: 'Master Setup',
  ...
  'fees_collection': {
    ...
    primaryKey: 'fee_id',
    description: 'Student fees, class-wise dues, late fines and receipts',
    fields: [
      f('class_name', 'Class name', 'select', true, [
        'PG',
        'NURSERY',
      ...
      rel('student_id', 'Student', 'student_master', 'student_id', 'full_name', true),
      f('admission_no', 'Admission number', 'text', true),
      f('student_name', 'Student name', 'text', true),
      f('academic_year', 'Academic year', 'select', false, ACADEMIC_YEAR_OPTIONS),
      f('due_month', 'Due month', 'select', true, [
        'April',
        'May',
        'June',
        'July',
      ...
      f('fees_amount', 'Fee amount (₹)', 'number', true),
      f('fine_amount', 'Fine amount (₹)', 'number'),
      f('fine_waived', 'Fine Waived', 'boolean'),
      f('principal_approval', 'Principal Approval', 'text'),
      f('waive_approved_by_principal', 'Approved by Principal (System)', 'boolean'),
      f('fine_waive_reason', 'Concession reason / notes', 'text'),
      f('approved_by', 'Approved authority', 'text'),
      f('amount_due', 'Amount due (₹)', 'number', true),
      f('amount_paid', 'Amount paid (₹)', 'number', true),
      f('payment_date', 'Payment date', 'date', true),
      ...
      f('remarks', 'Remarks / Notes', 'textarea'),
    ],
    columns: ['receipt_number', 'student_name', 'class_name', 'due_month', 'fee_type', 'fees_amount', 'fine_amount', 'amount_due', 'amount_paid', 'status'],
  },
  'expense_master': {
  ...
  'income_master': {
    ...
    description: 'School non-fee receipts, item sales and sundry income records',
    fields: [
      f('income_date', 'Income date', 'date', true),
      f('income_category', 'Income category', 'select', true, [
        'Fee Income',
        'Uniform and Educational Materials',
        'Extra-Curricular Income',
        'Grants & Donations',
      ...
      f('income_type', 'Income type / Head', 'select', true, [
        'Examination Fee',
        'Computer Fee',
        'Smart Class Fee',
        'Activity Fee',
...
export const label = (key: string) => {
  ...
  if (key === 'fees_structure') return 'Fees Structure'
  if (key === 'income_head_master') return 'Income Heads / Types'
